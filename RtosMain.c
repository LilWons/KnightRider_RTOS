/* 
Project Title: Knight Rider RTOS
Author: LilWons
Date Created: 2023-12-5

Revision History:
Dec 12/04: Fixed bug where CN interrupt signaled BINSEM_DIR_SWAP more than once.
Dec 12/05: Updated changeState to only allow CN ISR to call BINSEM_DIR_SWAP.
Dec 12/05: Increased KnightRider speed to 30 ms.
Dec 12/05: Changed priorities for ADC and UART tasks. 
*/
#include <xc.h>
#include <salvo.h>
#include <stdint.h>
...
volatile uint8_t changeState = 1;
////////////////////////////////////////////////////////////////////////////////
...
#define TASK_DIRECTION_SWAP_P         OSTCBP(6)     //Task #6
////////////////////////////////////////////////////////////////////////////////
//Defining task priority
#define PRIO_LED_TOGGLE             4           //Task1 -> Priority 4 
#define PRIO_SEND_TEMP              6         //Task2 -> Priority 9
#define PRIO_SEND_VOLTAGE           6         //Task3 -> Priority 9
#define PRIO_READ_AN                5          //Task4 -> Priority 8
#define PRIO_KNIGHT_RIDER           10          //Task5 -> Priority 10
#define PRIO_DIRECTION_SWAP         3          //Task6 -> Priority 10
////////////////////////////////////////////////////////////////////////////////
...
void U2_TxByte(char charValue)
{
    while (U2STAbits.UTXBF); // Wait for the transmit buffer to be empty.
    U2TXREG = charValue; // Write the character to the transmit buffer.
}
////////////////////////////////////////////////////////////////////////////////
...
void KnightRider(void){
    for(;;){
        ...
        OS_Delay(30); //Force context switch and delay task by 400 milliseconds.
        }
    }
////////////////////////////////////////////////////////////////////////////////
...
void DirectionSwap(void)
{
    for(;;){
       ...
       // changeState = 0; //Set changeState to zero.
...
void LED_Toggle (void)
{
 for (;;) 
 {
     ...
     OS_Delay(150); //Force context switch and delay task by 175 milliseconds.
     OS_Delay(150); //Force context switch and delay task by 175 milliseconds.
 }
}
////////////////////////////////////////////////////////////////////////////////
...
void __attribute__((__interrupt__, __auto_psv)) _CNInterrupt(void) {
    if(PORTDbits.RD6 == 0 && changeState)//If Switch 3 is pressed and changeState is 1...
    {
       changeState = 0; //Sets changeState to 0 once Switch 3 is pressed.
       OSSignalBinSem(BINSEM_DIR_SWAP);//Signal BINSEM_DIR_SWAP semaphore.
    }
    changeState = 1; //Sets changeState to 1 only after Switch 3 is released. 
...
void buttonInit(void) {
    ...
    IPC4bits.CNIP = 1; //Sets CN ISR Priority.
...
void TMR1_Initialize (void)
{
    ...
    PR1 = 0x0F9F; //Interrupt at = 1 ms; Frequency = 8MHz/2 = 4000000 Hz; T = 0.25 us; PR1= 3999.
    T1CON = 0x0000; //TCKPS 1:1; TON disabled; TSIDL disabled; TCS FOSC/2; TSYNC disabled; TGATE disabled.
